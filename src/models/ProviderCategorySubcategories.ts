/* tslint:disable */
/* eslint-disable */
/**
 * Torrents Stream Server
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: 1.0
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { exists, mapValues } from '../helpers';
/**
 * 
 * @export
 * @interface ProviderCategorySubcategories
 */
export interface ProviderCategorySubcategories {
    /**
     * 
     * @type {string}
     * @memberof ProviderCategorySubcategories
     */
    name: string;
    /**
     * 
     * @type {string}
     * @memberof ProviderCategorySubcategories
     */
    id: string;
}

export function ProviderCategorySubcategoriesFromJSON(json: any): ProviderCategorySubcategories {
    return ProviderCategorySubcategoriesFromJSONTyped(json, false);
}

export function ProviderCategorySubcategoriesFromJSONTyped(json: any, ignoreDiscriminator: boolean): ProviderCategorySubcategories {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        
        'name': json['name'],
        'id': json['id'],
    };
}

export function ProviderCategorySubcategoriesToJSON(value?: ProviderCategorySubcategories | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
        return null;
    }
    return {
        
        'name': value.name,
        'id': value.id,
    };
}


