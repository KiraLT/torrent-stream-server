...
export function GoogleDrivePopupWidget({url, name, open, onClose}: {
    url: string,
    name: string,
    open: boolean,
    onClose: () => void
...
export const GoogleDriveWidget = memo(({name, url}: {name: string, url: string}) => {
    ...
    return <>
        <Alert variant="info">
            Click on button below to start uploading. Closing popup will cancel upload process!
        </Alert>
